...
const ServerError = () => {
  ...
  const { error, isServerDown, isRecovering, retryCount, attemptRecovery, markServerRecovered } =
    useContext(ErrorContext);

  // Auto-check if server is back every 3 seconds
  useEffect(() => {
  ...
  }, [isServerDown, attemptRecovery, markServerRecovered]);
      ...
      <div className="bg-white rounded-lg shadow-2xl max-w-2xl w-full p-8">
        ...
        <div className="bg-yellow-50 border border-yellow-300 rounded-lg p-4 mb-6">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-yellow-900 font-semibold">Auto-Recovery Status</p>
              <p className="text-yellow-700 text-sm mt-1">
                {isRecovering ? (
                  <>
                    <span className="inline-block w-3 h-3 bg-yellow-500 rounded-full animate-pulse mr-2"></span>
                    Checking server... Attempt #{retryCount}
                  </>
                ) : (
                  <>
